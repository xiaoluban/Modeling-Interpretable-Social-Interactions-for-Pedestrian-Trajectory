...
def validation(dataloader, net, args, stgraph, epoch, log_file_curve):
    ...
    for batch in range(dataloader.num_batches):  # 81
        ...
        loss_batch_curr = loss_batch_curr / 8
        loss_epoch_cur += loss_batch_curr

        loss_batch2 = loss_batch2 / 8
        loss_epoch2 += loss_batch2

    loss_epoch = loss_epoch / dataloader.num_batches

    loss_epoch_cur = loss_epoch_cur / dataloader.num_batches
    loss_epoch2 = loss_epoch2 / dataloader.num_batches

    print(
        'epoch{}(batchnum{}), train_losss = {:.3f}, train_losss_curr = {:.3f}, train_losss2 = {:.3f}, time = {:.3f}'.format(
            epoch,
            81,
            loss_epoch, loss_epoch_cur, loss_epoch2,
            end - start))
